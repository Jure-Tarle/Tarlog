...
export default function Invoices() {
  ...
  return (
    <Page title={t("Rechnungen")} hint={t("Revisionsfähiger Rechnungsbestand")}>
      <Card
        title={t("Rechnungen")}
        subtitle={t("Erstellung, Finalisierung und PDF-Export erfolgen im Server-Modus (apps/web).")}
      >
        <AsyncBody
          state={{ data: list.data, error: list.error, loading: list.loading }}
          empty={<EmptyState title={t("Noch keine Rechnungen")}>{t("Rechnungen aus erfassten Zeiten erstellst du im Server-Modus.")}</EmptyState>}
        >
          {(rows) => (
            <TableWrap>
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th>{t("Typ")}</th>
                    <th>{t("Status")}</th>
                    <th className="right">{t("Netto")}</th>
                    <th className="right">{t("Brutto")}</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((inv) => (
                    <tr key={inv.id}>
                      <td className="num">{inv.invoice_number ?? <span className="faint">{t("Entwurf")}</span>}</td>
