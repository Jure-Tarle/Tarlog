...
import { Button, ButtonLink, Field, Select, TextInput } from "@/lib/ui/controls";
...
export default async function ReportsPage({
  ...
  try {
    ...
    filters = (
      <Card className="report-filter-card">
        <form action="/reports" method="get" className="report-filter-form">
          <Field label="Von">
          ...
          <Field label="Projekt">
            <Select name="project" defaultValue={sp.project ?? ""} className="report-filter-project">
              <option value="">alle</option>
              {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </Select>
          </Field>
          <Field label="Abrechenbarkeit">
            <Select name="billable" defaultValue={sp.billable ?? ""}>
              <option value="">alle</option>
  ...
  return (
    <section>
      <PageHeader title="Reports" subtitle={`Zeitraum ${from}, ${to}`} actions={<ButtonLink href="/exports">Als Datei exportieren</ButtonLink>} />
      {filters}
      {body}
    </section>
  );
